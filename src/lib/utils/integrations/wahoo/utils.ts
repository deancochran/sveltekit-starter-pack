...
export function convertTrainingSessionToWahooPlan(
  ...
  for (let i = 0; i < workout_intervals.length; i++) {
    ...
    if (workout_type_family == WAHOO_PLAN_FAMILY_TYPE.BIKING) interval_name = `${(interval.intensity * user.bike_ftp).toFixed(0)}w`
    if (workout_type_family == WAHOO_PLAN_FAMILY_TYPE.RUNNING) interval_name = `${secondsToHHMMSS(interval_s_p_km, false)}min/km`
