<script lang="ts">
	...
	import type { ActivityType, trainingSession } from '@prisma/client';
	...
	export let training_session: trainingSession;
	let intervals = training_session.plan as WorkoutInterval[];
	...
	{#each $items as item, index (item.id)}
		<div
			...
			style="width: {Math.ceil(
				(item.data.duration / training_session.duration) * 100
			...
			>
				<div class="flex flex-row items-center align-middle justify-center gap-1">
					<span class="text-sm text-nowrap"
						>{getIntervalDisplay(item.data, training_session.activity_type, $page.data.user)}</span
					>
				</div>
			</div>
		</div>
