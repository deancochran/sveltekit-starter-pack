...
<AppBar gridColumns="grid-cols-3" slotDefault="place-self-center" slotTrail="place-content-end">
	<svelte:fragment slot="lead">
		<Link class="btn" href="/">
		<h1 class="h1 font-serif font-bold text-4xl">
			<span
	...
	<svelte:fragment slot="trail">
		<nav class="w-full inline-flex justify-evenly space-x-1">
			{#if !session}
				<Link class="btn hover:variant-soft-primary" href="/sign-in">Sign-In</Link>
				<Link class="btn hover:variant-soft-primary" href="/sign-up">Sign-Up</Link>
			{/if}
			<ThemeSelector />
			<Link
